...
void syscall_handler() {
    printf(GREENSTR("Invoking system call [%llu]\n"), cpu_reg.rax);
...
static void print_kstack(pcb_t *p) {
    ...
    // general info
    printf("Kernel stack info [PID = %llu]\n" \
            "top vaddr:\t0x%016llx\n" \
            "bottom vaddr:\t0x%016llx\n" \
            "RSP now:\t0x%016llx\n" \
    ...
    for (int r = 0; r < 3; ++ r) {
        ...
        for (int c = 0; c < n_cols; ++ c) {
            ...
            if (tfa <= base) {
                ...
                printf(GREENSTR("%16llx  "), val64);
            } else if (ufa <= base) {
                // print user frame yellow
                printf(YELLOWSTR("%16llx  "), val64);
            } else {
                printf("%16llx  ", val64);
            }
            base -= size64;
    ...
    // trap frame
    printf("=====\nTrap frame: 0x%016llx\n", tfa);
    trapframe_t *tf = (trapframe_t *)tfa;
    printf("RIP: %016llx\nRSP: %016llx\n", tf->rip, tf->rsp);

    // user frame
    printf("=====\nUser frame: 0x%016llx\n", ufa);
