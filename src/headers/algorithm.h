#ifndef CSAPP_ALGORITHM_H
#define CSAPP_ALGORITHM_H
...
typedef struct {
    ...
    int counter;            // the counter of slots(hava data)
    char **karray;
    uint64_t *varray;
} hashtable_bucket_t;
 ...
 * are designed to be Generic. To use, the user should design
 * their own data structure and implement the interface with
 * function pointers. With this design, the actual data structure
 * can be very free. It can be malloced on heap. And it can be a
 * byte_block like in ./src/malloc
 */
...
typedef struct {
    ...
    /**
     * @param: "first": the id of first node
     * @param "second": the id of second node
     * @return 0 if they are the same
     */
    int (*compare_nodes)(uint64_t, uint64_t);
    /**
     * @param: the id of current node
     * @return: the id of the previous node
     */
    uint64_t (*get_node_prev)(uint64_t);
    /**
     * @param: "node": the id of current node
     * @param: "prev": the id of previous node
     * @return:1 if the setting is successful
     */
    ...
    /**
     * @param: "node": the id of current node
     * @param: "next": the id of next node
     * @return: 1 if the setting is successful
     */
...
typedef struct LINKEDLIST_INTERNAL_STRUCT {
    ...
    /**
     * @param: this pointer
     * @param "node": the id of new head node
     * @return 1 if the updating is successful
     */
 ...
 */
 ...
 int linkedlist_internal_delete(linkedlist_internal_t *list,
                                linkedlist_node_interface *i_node,
                                uint64_t node);
 int linkedlist_internal_index(linkedlist_internal_t *list,
                               linkedlist_node_interface *i_node,
                               uint64_t index);
